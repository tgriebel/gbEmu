...
void DrawTile( Bitmap& tileMap, const uint32_t xOffset, const uint32_t yOffset, const uint32_t tileId )
{
	const uint16_t baseAddr = 16 * tileId;
...
#define LOG_DEBUG 0
	...
	while( true )
	{
		...
		{
			string logBuffer;
			gbSystem.cpu.dbgLog.ToString( logBuffer, currentFrame, currentFrame + 1 );
			std::cout << "Log:\n" << logBuffer << std::endl;
...
#if TILEMAP_DEBUG
		{
			...
			for ( uint32_t tileId = 0; tileId < 128; ++tileId )
			{
				...
				DrawTile( tileMap, pixelOffsetX, pixelOffsetY, tileId );
			}

			stringstream ss;
			ss << "tilemap_" << currentFrame << ".bmp";
...
#if BG_DEBUG
		{
			Bitmap bgMap( 160, 144, 0x00000000 );

			for ( uint32_t tileY = 0; tileY < 18; ++tileY ) {
				for ( uint32_t tileX = 0; tileX < 20; ++tileX ) {
					const uint32_t pixelOffsetX = tileX * 8;
					const uint32_t pixelOffsetY = tileY * 8;

					const uint32_t tileId = gbSystem.vram[ 0x1800 + tileY * 18 + tileX ];
					DrawTile( bgMap, pixelOffsetX, pixelOffsetY, tileId );
				}
			}

			stringstream ss;
			ss << "bgmap_" << currentFrame << ".bmp";
